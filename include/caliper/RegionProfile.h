...
public:
    ...
    /// \brief Return exclusive nested path profile
    ///
    /// Return exclusive time in each encountered call path. Paths are
    /// encoded as strings with `/` separating regions in the hierarchy.
    region_profile_t
    exclusive_path_profile();

    /// \brief Return inclusive nested path profile
    region_profile_t
    inclusive_path_profile();
